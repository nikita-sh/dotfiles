...
- Rebuild home config: `hmrb switch`. Inspect `alias hmrb` for the active host flake; private host configurations can live outside this repository.
- Rebuild system config: `nixosrb switch`.
- Changes under `shared/` need a re-lock before they reach a rebuild. Host flakes pin the local `shared` input, so `hmrb switch` can install the locked version while your edit is still absent. Stage changed tracked files, run `nix flake update shared` in the active host flake directory, then `hmrb switch`. Confirm by inspecting the rendered target under the harness's config directory, not the source.
